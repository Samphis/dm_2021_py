...
def DIV_NN_Dk(mas1, mas2): #Вычисление первой цифры деления большего натурального на меньшее,
    ...
    for i in range(mas1[0][0]):
        num1 = num1 + mas1[1][i] * (10 ** (mas1[0][0] - i - 1))
    for i in range(mas2[0][0]):
        num2 = num2 + mas2[1][i] * (10 ** (mas2[0][0] - i - 1)) #сборка num1 и num2 из массивов mas1 и mas2
